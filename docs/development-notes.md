...
[grid_hash (`grid_hash.h`)](#grid_hash-grid_hashh)
...
initializing it in `init_grid_hash_mask.cpp`. This will store it in the game's
`type_table_t` struct. It can then be accessed by the
