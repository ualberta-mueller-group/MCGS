...
poly_serializable_id_t next_sid = 1;
...
{
    poly_serializable_id_t& sid = type_table->poly_serializable_id_ref();
    THROW_ASSERT(sid == 0);
    sid = next_sid++;
    load_functions.push_back(load_function);

    assert(get_load_function(type_table->poly_serializable_id()) ==
           load_function);
}
