...
shared_ptr<ThGraph> thermograph_builder_no_db::build_thermograph(sumgame& sum)
{
    // Generate options
    ...
    vector<shared_ptr<ThGraph>> option_graphs_b =
        _get_option_graphs_for_player(sum, BLACK);
    vector<shared_ptr<ThGraph>> option_graphs_w =
        _get_option_graphs_for_player(sum, WHITE);
    ...
    sumgame& sum)
{
    ...
    shared_ptr<ThGraph> graph = build_thermograph(sum);
    ...
    _get_option_graphs_for_player(sumgame& sum, bw player)
{
    ...
    while (*gen)
    {
        ...
        shared_ptr<ThGraph> option_graph = _get_thermograph_from_cache(sum);
