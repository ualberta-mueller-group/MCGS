...
struct serializer<type_mapper>
{
    ...
    {
        serializer<decltype(mapper._next_disk_type)>::save(
            os, mapper._next_disk_type);
    ...
    inline static type_mapper load(ibuffer& is)
    {
        ...
        mapper._next_disk_type =
            serializer<decltype(mapper._next_disk_type)>::load(is);
...
inline game_type_t type_mapper::translate_type(game_type_t runtime_type) const
{
    ...
    // THROW_ASSERT(runtime_type < _type_remappings.size() &&
    // _type_remappings[runtime_type] > 0);
