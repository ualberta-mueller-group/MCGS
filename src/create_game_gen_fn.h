...
template <class Gridlike_Game_T>
create_game_gen_fn_t get_gridlike_create_game_gen_fn(
    const std::vector<int>& tile_sequence)
{
    static_assert(std::is_base_of_v<grid, Gridlike_Game_T> ||
                  std::is_base_of_v<strip, Gridlike_Game_T>);
    ...
    {
        constexpr bool STRIPS_ONLY = std::is_base_of_v<strip, Gridlike_Game_T>;
        ...
        return new gridlike_db_game_generator<Gridlike_Game_T>(gg);
    };
