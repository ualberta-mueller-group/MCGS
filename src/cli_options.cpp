...
cli_options::cli_options()
    : parser(nullptr),
      ...
      use_player(false),
      print_moves_action(PRINT_MOVES_ACTION_NONE),
      format_moves_as_options(false),
      test_directory(get_default_input_path()),
...
void print_help_message(const string& exec_name)
{
    ...
    print_flag(
        "--print-winning-moves",
        "Print winning moves for each player, or, if the sum is impartial, for "
        "one player. Resulting moves are first sorted and deduplicated.");

    print_flag("--print-subgame-moves",
               "For each subgame in the sum, print each players moves. "
               "Resulting moves are printed as generated by the underlying "
               "move_generators, and are not deduplicated.");

    print_flag(
        "--print-subgame-options",
        "Like \"--print-subgame-moves\", but prints games instead of board "
        "coordinates.");

    print_flag("--print-sum-moves",
               "Print each player's moves in the sum. Resulting moves are "
               "printed as generated by the underlying sumgame_move_generator, "
    ...
    print_flag("--run-tests",
               "Run all autotests. By default, reads tests from \"" +
                   path_relative_to_cwd(get_default_input_path()).string() +
    ...
    print_flag("--out-file <file name>",
               ...
               "from --run-tests. Default is \"" +
                   path_relative_to_cwd(get_default_csv_path()).string() + "\".");
    ...
    */
    ...
    for (arg_idx = 1; arg_idx < arg_n; arg_idx++) // skip "./MCGS"
    {
        ...
        if (arg == "--print-subgame-options")
        {
            ...
            opts.print_moves_action = PRINT_MOVES_ACTION_SUBGAME;
            opts.format_moves_as_options = true;
            continue;
        }
