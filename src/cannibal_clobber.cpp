...
{
public:
    cannibal_clobber_move_generator(const cannibal_clobber& game, bw to_play);
...
void cannibal_clobber::play(const ::move& m, bw to_play)
{
    game::play(m, to_play);

    int_pair from_coord, to_coord;
    bw target_color;
    cannibal_clobber_move::unpack_coords(m, from_coord, to_coord, target_color);
    ...
    const int from_point_color = at(from_point);
    const int to_point_color = at(to_point);

    assert(from_point_color == to_play);
    assert(to_point_color == target_color && is_black_white(to_point_color));
...
void cannibal_clobber::undo_move()
{
    ...
    int_pair from_coord, to_coord;
    bw target_color;
    cannibal_clobber_move::unpack_coords(m_enc, from_coord, to_coord,
                                         target_color);
...
dyn_serializable* cannibal_clobber::load_impl(i_ibuffer& is, serializer_ctx* ctx)
{
...
bool cannibal_clobber::is_move(const int& from, const int& to, bw to_play) const
{
    ...
    return (at(from) == to_play) && (is_black_white(at(to)));
}
...
split_result cannibal_clobber::_split_impl() const
{
         ...
         start.increment_position())
    {
        ...
        const int start_color = at(start_point);
    ...
    for (size_t i = 0; i < n_components; i++)
    {
        ...
        cannibal_clobber* g_new = nullptr;
        ...
        if (!different)
        {
            ...
            g_new = new cannibal_clobber(board_untrimmed, grid_shape);
        }
        else
        {
            assert(!grid_location::shape_is_empty(new_shape));
            g_new = new cannibal_clobber(new_board, new_shape);
        }
...
{
    str << "cannibal_clobber:" << board_as_string();
}

void cannibal_clobber::print_move(std::ostream& str, const ::move& m, ebw to_play) const
{
    assert(is_black_white(to_play));
    // (from, to)
    cannibal_clobber_move::print_as_coords(str, m, shape());
}

game* cannibal_clobber::inverse() const
{
...
void cannibal_clobber_move_generator::operator++()
{
    assert(*this);
    _next_move(false);
}

cannibal_clobber_move_generator::operator bool() const
{
    return _has_move;
}

::move cannibal_clobber_move_generator::gen_move() const
{
    assert(*this);
    assert(_location.valid() && _target_location.valid());
    assert(is_black_white(_target_color));

    const int_pair& from_coords = _location.get_coord();
    const int_pair& to_coords = _target_location.get_coord();

    return cannibal_clobber_move::create_from_coords(from_coords, to_coords,
                                                     _target_color);
}

void cannibal_clobber_move_generator::_next_move(bool init)
{
    assert(init || *this);
    ...
    while (true)
    {
        ...
        if (valid_neighbor)
        {
            ...
            _target_color = _game.at(_target_point);
...
inline bool cannibal_clobber_move_generator::_increment()
{
    if (_increment_dir())
        return true;
...
inline bool cannibal_clobber_move_generator::_increment_dir()
{
    assert(_dir_idx < GRID_DIRS_CARDINAL.size());
    _dir_idx++;
