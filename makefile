...
# Basic checks
DEBUG_FLAGS_COMMON := -g -DASSERT_RESTORE_DEBUG -DGAME_UNDO_DEBUG

# Additional checks that are expensive and/or annoying
DEBUG_FLAGS_EXTRA := -DSUMGAME_DEBUG -DTYPE_TABLE_DEBUG -DDEFAULT_IMPL_DEBUG -D_GLIBCXX_DEBUG -DNOGO_DEBUG
...
ifneq (,$(filter $(USE_DOMINANCE),1 true))
	NORMAL_FLAGS := $(NORMAL_FLAGS) -DMCGS_USE_DOMINANCE
	TEST_FLAGS := $(TEST_FLAGS) -DMCGS_USE_DOMINANCE
endif

ifneq (,$(filter $(USE_DB_MOVES_TEST),1 true))
	NORMAL_FLAGS := $(NORMAL_FLAGS) -DMCGS_USE_DB_MOVES_TEST
	TEST_FLAGS := $(TEST_FLAGS) -DMCGS_USE_DB_MOVES_TEST
endif
